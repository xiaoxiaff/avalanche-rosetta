...
func TestCrossChainSkippedOs(t *testing.T) {
	t.Run("Export tx skipped op", func(t *testing.T) {
		var (
			rawIdx      = 0
			...
			pChainBlockID, _     = ids.FromString("11111111111111111111111111111111LpoYY")
			...
			// metaID, _            = ids.FromString("7QUPqUAMdny53bVptZ2DgxLLN4qZ5X7MnBPseUKYnoh5C5v47")
			meta = &avax.Metadata{}
		)
		...
		assert.Nil(t, err)
		ops, skippedOps, err := crossChainTransaction(rawIdx, avaxAssetID, tx, networkIdentifier, &pChainBlockID)
		assert.Nil(t, err)
